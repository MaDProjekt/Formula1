...
int energia = 0; // Počiatočná energia
int maxEnergia = 100; // Maximálna energia, ktorú môžete mať
...
void loop() {
  ...
  int motorSpeed = map(ps2x.Analog(PSS_RY), 0, 255, -200, 200); // Meníme rozsah rýchlosti

  if (!isButtonPressed) {
  motorSpeed = map(motorSpeed, -200, 200, -200, 200); // Nastavíme rýchlosť na 100% ak nie je tlačidlo stlačené
  } else {
    motorSpeed = map(motorSpeed, -200, 200, -150, 150); // Obmedzíme rýchlosť na 80% ak je tlačidlo stlačené
  }
  ...
  if (isButtonPressed) {
    if (energia < maxEnergia) {
      ...
      // Tu môžete pridať efekty, LED signalizáciu alebo iné indikácie nabíjania
    } else {
      Serial.println("Energia je plná!");
    }
  } else {
    if (energia > 0) {
      energia--; // Odoberieme energiu
      Serial.println("Vybíjanie...");
      // Tu môžete pridať efekty, LED signalizáciu alebo iné indikácie vybíjania
    } else {
      Serial.println("Energia je úplne vybitá!");
    }
  }

  // Výpočet percentuálnej hodnoty energie
  int percentoEnergie = (energia * 100) / maxEnergia;
  Serial.print("Percento energie: ");
  Serial.print(percentoEnergie);
  Serial.println("%");

  // Kontrola a akcie na základe stavu časovača
  ...
  if (casovacKoniec > 0 && !isButtonPressed) {
    if (teraz < casovacKoniec) {
      ...
      // Tu môžete vykonávať ďalšie akcie na základe zostávajúceho času časovača
    } else {
